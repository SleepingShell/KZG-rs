use std::{fmt, marker::PhantomData, collections::BTreeMap, ops::{Mul, Div}, error::Error};

use ark_crypto_primitives::sponge::CryptographicSponge;
use ark_ec::{Group, pairing::Pairing};
use ark_ff::{Field, One, PrimeField, Zero};
use ark_poly::{DenseUVPolynomial, GeneralEvaluationDomain, EvaluationDomain, Evaluations};
    ...
    KZGUniversalParams<G1, G2>
{
    ...
    fn trim(&self, trim_degree: usize) -> Self {
        Self {
            degree: trim_degree,
            ref_string_g1: self.ref_string_g1[0..trim_degree].to_vec(),
            ref_string_g2: self.ref_string_g2[0..trim_degree].to_vec(),
        }
    }
...
#[derive(Clone, Debug, CanonicalSerialize, CanonicalDeserialize)]
struct KZGRandomness<F: PrimeField, P: DenseUVPolynomial<F, Point = F>> {
    blinding_polynomial: P,

    _f: PhantomData<F>
}

impl<F: PrimeField, P: DenseUVPolynomial<F, Point = F>> PCRandomness for KZGRandomness<F,P> {
    fn empty() -> Self {
        Self {
    ...
    fn rand<R: RngCore>(
        ...
        random.blinding_polynomial = P::rand(num_queries, rng);
        random
    }
}
    ...
    for<'p> &'p P: Div<&'p P, Output = P>,
{
    ...
    type Randomness = KZGRandomness<E::ScalarField, P>;
    ...
    // TODO: rng can be used to create a perfectly hidden polynomial (PolyCommit_Ped in KZG paper)
    fn commit<'a>(
        ...
        Self::Commitment: 'a,
    {
        ...
        let key_sub_index = vk.element_at_g2(1) - (<E::G2 as Group>::generator() * point);
